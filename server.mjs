...
const LINK_CONTAINER_SELECTOR =
  process.env.LINK_CONTAINER_SELECTOR || 'main, [role="main"], #content, body';
...
const ASSIGN_FORCE_LIST_BUTTON =
  ...
  || '[aria-label="List view"], [title*="List"], button[aria-label*="List"], button[title*="List"], .bb-icon-list, [data-automation-id*="list-view"], [data-view="list"]';

// Google Drive (optional)
const SA_JSON         = process.env.GOOGLE_SERVICE_ACCOUNT_JSON || "";
const GDRIVE_FOLDERID = process.env.GOOGLE_SERVICE_ACCOUNT_FOLDER_ID || process.env.GDRIVE_FOLDER_ID || "";
...
async function loginBlackbaud(page) {
  ...
  // (1) “Sierra Canyon School” SSO button (if present)
...
async function scrapeAssignments() {
  ...
  try {
    ...
    // ---- open each detail page and extract ----
    const assignments = [];
